 ...
 *    `color-mix()`. The hero's "why it matters" card here is a `color-mix(in
 *    oklab, var(--client) 6%, transparent)` wash, which axe files under
 *    `incomplete` rather than measuring.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen, and this lab leans on
 *    opacity harder than most. Every unreached `.step-item` in the walkthrough
 *    renders at `.35` and every completed one at `.6`; disabled buttons drop to
 *    `.4`; the footer is an inline `opacity:.85` block whose scripture line
 *    fades again to `.75`; the hero subtitle sits at `.85`. Note this lab
 *    collapses animations to 0.01ms under reduced motion and cancels the
 *    on-path pulse outright, which preserves end states — that pulse loops
 *    indefinitely otherwise and would never let a scan settle.
 *
 ...
 *    nine scrollers: `.pm-scroll` caps the 16-row position map at 14rem, the two
 *    `.access-log` panels cap 20 access rows at 280px, `.tree-container` clips a
 *    600px-floor tree, and the five `.scenario-wrap` boxes clip `white-space:
 *    pre` logs. Content scrolled past those boxes is not dimmed or partly drawn
 ...
 *    failure. Skip it, and rely on the rows that ARE in view, which are measured
 *    for real.
 *
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This rule
 *    is load-bearing here, not a spare part: `renderTree` emits each block slot
 *    as a `<rect class="block-real">` (or `.block-dummy`) and then that slot's
 *    id as a FOLLOWING `<text class="block-id-label">`, and each bucket's `B12`
 *    caption after the bucket rect. Those labels' backdrops are preceding
 *    sibling shapes, not ancestors. An ancestor-only walk would composite them
 *    onto the svg's own transparent background — i.e. straight onto the card —
 *    and report ratios nothing on screen has. So for SVG text, any earlier
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" card is authored with `color-mix(in oklab, ...)`, and Chromium reports that
     ...
     * gradient whose worst stop spans its element; judged that way, a page-wide
     * wash that fades to `transparent` invents backdrops for text a full page
     * below it (accent links read 3.56:1 elsewhere in this fleet where they
     * actually render ~4.9:1). So each gradient is *sampled* at the text's real
     * location instead: linear by projecting onto the gradient line, radial by
     * distance from the centre over the farthest-corner radius. A non-gradient
     * layer (`url()`, `none`) is unmeasurable and paints nothing. This lab uses
     * no gradients today; the branch is kept because it is silent when unused
     * and the alternative is a helper that mismeasures the first one added.
     */
     ...
     *
     * A driven scan here walks two 31-bucket trees with 124 block slots each,
     * a 16-row position map, twenty access rows in each of two logs, seven
     * walkthrough steps and a 16-line chi-square histogram, and every one of
     * those nodes re-walks the same handful of ancestors up to `<body>`.
     * Without the caches the pass re-reads the same computed styles and rects
     * thousands of times, and the gate scans two dozen states per
     * configuration. Nothing mutates the DOM during the pass, so the cached
     * values cannot go stale — which is also why the gate never scans while
     * auto-run is stepping.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // opaque black rectangle covering whatever it crosses. That matters here:
      // `renderTree` draws a `<line>` from each bucket to its parent before the
      // bucket rects, and compositing those as opaque black invented a 3.82:1
      // failure elsewhere in this fleet for labels whose real ratio was 6.15:1.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has no <details>; what it has is four exhibit
      // panels carrying the `hidden` attribute at any moment, plus a
      // `section.exhibit { display: none }` rule and one `.acc-client` span set
      // to `display:none` inline in every server-log row. `checkVisibility()`
      // catches all of those, and the gate opens each exhibit through its own
      // tab rather than un-hiding it.
     ...
     * it, the `aria-hidden` decoration this lab carries — the walkthrough's
     * step numerals, the position map's remap arrow and the costs table's
     * footnote asterisk — would be measured as if each took its element's
     * declared `color`, reporting ratios for marks that carry no text at all.
     */
     ...
     *
     * This lab's tree is built by concatenating markup into one string, so
     * there are no stray text nodes today. The guard stays because the mistake
